...
import { getCampusSettingsService } from '@/modules/campus-settings/services/campus-settings.service'
...
export default function TimetablePreviewPage() {
  ...
  // Title/watermark now live in Campus Settings — fetched read-only here and
  // applied automatically. Managed via the Campus Settings page, not this one.
  const [brandingTitleUrl, setBrandingTitleUrl]         = useState(null)
  const [brandingWatermarkUrl, setBrandingWatermarkUrl] = useState(null)

  useEffect(() => {
    getCampusSettingsService(campusId).then((result) => {
      if (result.success) {
        setBrandingTitleUrl(result.data.resolved.active_title_url)
        setBrandingWatermarkUrl(result.data.resolved.watermark_url)
      }
    })
  }, [campusId])
              ...
              : classData
                ? (
                  <ClassWisePreview
                    ...
                    rows={classData.rows}
                    printRef={gridRef}
                    titleUrl={brandingTitleUrl}
                    watermarkUrl={brandingWatermarkUrl}
                  />
                )
                : null}
        </TabsContent>
